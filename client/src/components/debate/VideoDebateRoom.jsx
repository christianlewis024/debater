...
import { subscribeToDebateState, switchTurn, startDebate, initializeDebateState } from '../../services/debateStateService';

const VideoDebateRoom = ({ debateId, participants, currentUser, userProfile, debate }) => {
  ...
  const isDebater = participants.debater_a?.userId === currentUser?.uid || 
                    participants.debater_b?.userId === currentUser?.uid;
  
  const isModerator = participants.moderator?.userId === currentUser?.uid;
  
  const myRole = participants.debater_a?.userId === currentUser?.uid ? 'debater_a' : 
  ...
  useEffect(() => {
    const initDebateState = async () => {
      if (participants.debater_a && participants.debater_b && debate?.settings) {
        console.log('Initializing debate state...', { debateId, settings: debate.settings });
        try {
  ...
  // Timer countdown
  useEffect(() => {
    if (!debateState || !debateState.debateStarted || debateState.debateEnded || debateState.paused) {
  ...
  const createTracks = async () => {
    const audioConfig = selectedMicrophone ? { microphoneId: selectedMicrophone } : {};
  ...
  const joinChannel = async () => {
    if (!currentUser || !debateId || !appId || joining || hasJoinedRef.current) return;
  ...
  useEffect(() => {
    if (currentUser && !joined && !joining && debateId && !hasJoinedRef.current && isDebater) {
  ...
  if (!currentUser) {
    return (
      <div style={{ 
        ...
        <p style={{ color: '#94a3b8', fontSize: '16px' }}>Login to join video chat</p>
      </div>
    );
  }
    ...
    <div 
      ...
      <div style={{ 
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
          ...
          {debateState && debateState.debateStarted && !debateState.debateEnded && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
              <div style={{
                padding: '8px 16px',
                background: isMyTurn ? 'rgba(16, 185, 129, 0.2)' : 'rgba(100, 116, 139, 0.2)',
                borderRadius: '10px',
                border: `1px solid ${isMyTurn ? 'rgba(16, 185, 129, 0.4)' : 'rgba(100, 116, 139, 0.3)'}`,
                display: 'flex',
                alignItems: 'center',
                gap: '8px'
              }}>
                <span style={{ fontSize: '18px' }}>{isMyTurn ? '🎤' : '👂'}</span>
                <span style={{ 
              ...
              <div style={{
                padding: '8px 20px',
                background: timeRemaining <= 10 ? 'rgba(239, 68, 68, 0.2)' : 'rgba(59, 130, 246, 0.2)',
                borderRadius: '10px',
                border: `1px solid ${timeRemaining <= 10 ? 'rgba(239, 68, 68, 0.4)' : 'rgba(59, 130, 246, 0.4)'}`,
                fontWeight: '800',
                fontSize: '18px',
          ...
          <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
            ...
            {!debateState && participants.debater_a && participants.debater_b && debate && (
              <button
                onClick={async () => {
                  console.log('Manual init clicked');
                  try {
                    await initializeDebateState(debateId, debate.settings || { turnTime: 60, maxTurns: 10 });
            ...
            {/* Start button */}
            {debateState && !debateState.debateStarted && participants.debater_a && participants.debater_b && (
              <button
        ...
        <div style={{ display: 'flex', gap: '10px' }}>
            ...
            <>
              <button
                ...
                style={{
                  ...
                  background: micMuted ? 'linear-gradient(135deg, #ef4444 0%, #dc2626 100%)' : 'rgba(255, 255, 255, 0.1)',
                  color: '#fff',
                  boxShadow: micMuted ? '0 4px 15px rgba(239, 68, 68, 0.4)' : 'none'
                }}
              >
                {micMuted ? '🔇' : '🎤'}
              </button>
              <button
                ...
                style={{
                  ...
                  cursor: 'pointer',
                  transition: 'all 0.2s ease',
                  background: cameraMuted ? 'linear-gradient(135deg, #ef4444 0%, #dc2626 100%)' : 'rgba(255, 255, 255, 0.1)',
                  color: '#fff',
                  boxShadow: cameraMuted ? '0 4px 15px rgba(239, 68, 68, 0.4)' : 'none'
                }}
              >
                {cameraMuted ? '📹❌' : '📹'}
              </button>
              <button
                ...
                style={{
                  ...
                  cursor: 'pointer',
                  transition: 'all 0.2s ease',
                  background: showDeviceSettings ? 'rgba(59, 130, 246, 0.2)' : 'rgba(255, 255, 255, 0.1)',
                  color: '#fff'
                }}
              >
                ⚙️
          ...
          <button
            ...
            style={{
              ...
              cursor: 'pointer',
              transition: 'all 0.2s ease',
              background: isFullscreen ? 'rgba(59, 130, 246, 0.2)' : 'rgba(255, 255, 255, 0.1)',
              color: '#fff'
            }}
          >
            {isFullscreen ? '⛶' : '⛶'}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
              ...
              <div style={{ 
                ...
                {localVideoTrack && (
                  <div style={{ 
                    ...
                    {localSpeaking && <span style={{ width: '8px', height: '8px', background: '#10b981', borderRadius: '50%', animation: 'pulse 1s infinite' }}></span>}
                    You
                  </div>
                )}
                {!localVideoTrack && (
                  <div style={{ 
                    ...
                    <div style={{ textAlign: 'center' }}>
                      ...
                      <p style={{ color: '#64748b', fontSize: '14px', fontWeight: '500' }}>Loading camera...</p>
                    </div>
                  </div>
                )}
              </div>
              {/* Debater Info Card */}
              {myRole && (myRole === 'debater_a' ? participants.debater_a : participants.debater_b) && (
                <div style={{ 
                  marginTop: '12px',
                  display: 'flex',
                  alignItems: 'center',
                  ...
                  border: myRole === 'debater_a' 
                    ? '1px solid rgba(59, 130, 246, 0.3)' 
                    : '1px solid rgba(239, 68, 68, 0.3)',
                  borderRadius: '16px'
                }}>
                  <img
                    src={(myRole === 'debater_a' ? participants.debater_a : participants.debater_b).profileData?.photoURL}
                    alt={(myRole === 'debater_a' ? participants.debater_a : participants.debater_b).profileData?.username}
                    style={{ 
                      width: '64px', 
                      height: '64px', 
                      borderRadius: '16px',
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ 
                      ...
                      {(myRole === 'debater_a' ? participants.debater_a : participants.debater_b).sideDescription}
                    </div>
                  </div>
                </div>
              )}
          ...
          {remoteUsers.length > 0 ? (
            remoteUsers.map((user) => {
              ...
              const isDebaterA = participants.debater_a?.userId === user.uid.toString();
              const isDebaterB = participants.debater_b?.userId === user.uid.toString();
              const remoteDebater = isDebaterA ? participants.debater_a : isDebaterB ? participants.debater_b : null;
                ...
                <div key={user.uid}>
                  <div style={{ 
                    ...
                    <div style={{ 
                      ...
                      {remoteSpeaking[user.uid] && <span style={{ width: '8px', height: '8px', background: '#10b981', borderRadius: '50%', animation: 'pulse 1s infinite' }}></span>}
                      Opponent
                    </div>
                  </div>
            ...
            <div style={{ 
              ...
              <div style={{ textAlign: 'center' }}>
                ...
                <p style={{ color: '#64748b', fontSize: '16px', fontWeight: '600' }}>Waiting for opponent...</p>
              </div>
            </div>
          ) : null}
          ...
          {!isDebater && remoteUsers.length === 0 && (
            <div style={{ 
              ...
              <div style={{ textAlign: 'center' }}>
                ...
                <p style={{ color: '#64748b', fontSize: '18px', fontWeight: '600', marginBottom: '8px' }}>Waiting for debaters...</p>
                <p style={{ color: '#475569', fontSize: '14px' }}>You're watching as a viewer</p>
              </div>
            </div>
          )}
        </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px', flex: 1 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{
                  ...
                  {debateState?.paused ? '⏸️ Debate Paused' : 'Overseeing debate'}
                </div>
              </div>
            </div>

            {/* Moderator Controls - Only visible to moderator */}
            {isModerator && debateState && debateState.debateStarted && !debateState.debateEnded && (
              <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                {/* Pause/Resume Button */}
                <button
                  onClick={async () => {
                    const { pauseDebate, resumeDebate } = await import('../../services/debateStateService');
                ...
                <button
                  onClick={async () => {
                    const { addTime } = await import('../../services/debateStateService');
                ...
                <button
                  onClick={async () => {
                    const { switchTurn } = await import('../../services/debateStateService');
                ...
                <button
                  onClick={async () => {
                    if (window.confirm('Are you sure you want to end this debate?')) {
                      const { endDebate } = await import('../../services/debateStateService');
